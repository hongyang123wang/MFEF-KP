...
from nets.resnet50 import resnet50, resnet50_Head


class CenterNet_Resnet50(nn.Module):
    def __init__(self, num_classes=20, pretrained=False, poly_setting=None):
        ...
        #                -> 128, 128, 64 -> 128, 128, 2
        # -----------------------------------------------------------------#
        self.head = resnet50_Head(inchannel=64, outchannel=8, num_classes=num_classes, poly_setting=poly_setting)
    ...
    def forward(self, x):
        feat, kp = self.backbone(x)
        return self.head(feat), kp
